## SPiCT Explorer — turn a knob and see the consequence, without editing a script.
## {{STOCK_NAME_SENTENCE}} (*{{STOCK_LATIN}}*), ICES subareas {{ICES_AREAS}}
##
## Launch from the project root with:
##   shiny::runApp("shiny/spict-explorer")
##
## The app reads the same compiled series as the assessment data report, so what it shows is
## what a re-render would produce. It does not write anything back: when a configuration is
## worth keeping, put it in config.yaml or in R/explore_fit.R.

cran_pkgs <- c(
  "shiny", "bslib", "shinycssloaders", "dplyr", "tidyr",
  "readr", "ggplot2", "zoo", "DT", "tibble", "cowplot", "yaml", "here"
)
...
if (!requireNamespace("spict", quietly = TRUE)) {
  message("Installing spict from GitHub (DTUAqua/spict)…")
  if (!requireNamespace("remotes", quietly = TRUE)) install.packages("remotes", quiet = TRUE)
  remotes::install_github("DTUAqua/spict/spict", quiet = TRUE)
}
...
# shiny::runApp() sets the working directory to the app folder, so walk up to the project root
# marked by .here or config.yaml — the same anchors R/0_setup.R and docs/render.R use.
locate_project_root <- function() {
  d <- normalizePath(getwd(), mustWork = FALSE)
  for (i in 1:6) {
    if (file.exists(file.path(d, "config.yaml")) || file.exists(file.path(d, ".here"))) return(d)
    parent <- dirname(d)
    if (parent == d) break
    d <- parent
  }
  stop(
    "Could not locate the project root (looked for config.yaml or .here). ",
    "Launch with shiny::runApp(\"shiny/spict-explorer\") from the project root."
  )
}

PROJECT_ROOT <- locate_project_root()
setwd(PROJECT_ROOT)
source(file.path(PROJECT_ROOT, "R", "0_setup.R"))
source(file.path(PROJECT_ROOT, "R", "spict_helpers.R"))
source(file.path(PROJECT_ROOT, "R", "advice_tables.R"))
source(file.path(PROJECT_ROOT, "R", "ices_plots.R"))

CATCH_PATH <- file.path(output_dir, paste0(cfg$STOCK_SLUG, "_catches.rds"))
INDEX_PATH <- file.path(output_dir, paste0(cfg$STOCK_SLUG, "_index.rds"))

for (f in c(CATCH_PATH, INDEX_PATH)) {
  if (!file.exists(f)) {
    stop(
      "Missing ", basename(f), ". Run R/1_process_catches.R and R/2_prepare_index.R first."
    )
  }
}
...
# ── Data ───────────────────────────────────────────────────────────────────────────────────
# The same compiled series the assessment data report is fitted to, so what the app shows is
# what a re-render would produce.
load_catches <- function() readRDS(CATCH_PATH)
...
INDEX_NAME_DEFAULT <- cfg$INDEX_NAME %||% "Abundance index"

# ── Defaults, read from config.yaml ────────────────────────────────────────────────────────
# The starting point is the project's configured assessment, so the first thing the app shows
# is the assessment itself, and every knob turned from here is a departure from it.
prior_default <- function(key, fallback_mean, fallback_sd) {
  v <- cfg[[key]]
  if (is.null(v)) return(list(mean = fallback_mean, sd = fallback_sd, on = TRUE))
  v <- as.numeric(unlist(v))
  list(mean = exp(v[1]), sd = v[2], on = v[3] == 1)
}

DEFAULTS <- list(
  logbkfrac = prior_default("PRIOR_LOGBKFRAC", 0.9, 0.5),
  logr = prior_default("PRIOR_LOGR", 0.064, 0.5),
  logsdb = prior_default("PRIOR_LOGSDB", 0.05, 0.2),
  ...
  # timing
  catch_dtc = 1,
  index_month = round((cfg$INDEX_TIMING %||% 0.5) * 12),
  # catch uncertainty ramp
  stdev_high_year = cfg$STDEV_HIGH_YEAR,
  stdev_low_year = cfg$STDEV_LOW_YEAR,
  stdev_high_val = cfg$STDEV_FAC_HIGH %||% 3,
  schaefer = identical(tolower(cfg$PRODUCTION_MODEL %||% "schaefer"), "schaefer")
)
...
# ── UI ─────────────────────────────────────────────────────────────────────
ui <- page_navbar(
  title = "SPiCT Explorer — {{STOCK_NAME_SENTENCE}}",
